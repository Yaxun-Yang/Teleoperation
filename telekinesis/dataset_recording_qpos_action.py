...
class SystemPybulletIK:
    def __init__(self):
        ...
        kinova_path_src = "/media/yaxun/manipulation1/leaphandproject_ws/src/kinova-ros/kinova_description/urdf/robot.urdf"
        ...
        while True:
            ...
            while True:
                ...
                if transformations and "r" in transformations:
                    ...
                    kinova_qpos = self.compute_IK(final_pos, rot, target_pos, target_quat, arm_moving)
                    # recording dataset
                    ...
                    if (
                        ...
                        and kinova_qpos is not None
                    ):
                        print("="*10, "recording", "="*10)
                        color = capture.color[..., :3].astype(np.uint8)
                        color = cv2.resize(
                            color, (W, H), interpolation=cv2.INTER_NEAREST)
                        depth = capture.transformed_depth.astype(
                            np.float32) / 1e3
                        depth = cv2.resize(
                            depth, (W, H), interpolation=cv2.INTER_NEAREST)
                        depth[(depth < 0.01) | (depth >= zfar)] = 0
                        episode_data["obs/agentview_image"] += [color]
                        episode_data["obs/depth_image"] += [depth]
                        ...
                        target_pos[0] -= 0.5
                        kinova_qpos = np.array(kinova_qpos)
                        episode_data["actions"] += [
                            np.concatenate(
                                [kinova_qpos, self.hand_q])
                        ]
