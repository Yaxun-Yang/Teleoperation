...
class SimulatorDiffusionPolicy:
    ...
    def preprocess_image(self, image):
        """Preprocess camera image for the model, split into left and right halves"""
        if image is None:
            return None, None
        
        # Split image into left and right halves
        width = image.shape[1]
        image_left = image[:, :width//2, :]
        image_right = image[:, width//2:, :]
        
        # Resize both images to target size
        image_left_resized = cv2.resize(image_left, config.IMAGE_SIZE)
        image_right_resized = cv2.resize(image_right, config.IMAGE_SIZE)
        
        # Normalize to [0,1] range
        normalized_left = torch.tensor(image_left_resized).permute(2, 0, 1).float() / 255.0
        normalized_right = torch.tensor(image_right_resized).permute(2, 0, 1).float() / 255.0
    ...
    def get_observation_from_dataset(self, timestep):
        ...
        image_left, image_right = self.preprocess_image(self.obs_data['agentview_image'][timestep])
        
        obs = {
            'agentview_image_left': image_left,
            'agentview_image_right': image_right,
    ...
    def prepare_model_input(self):
        ...
        if self.mode == 'pose':
            ...
            if self.past_obs is not None:
                model_input = {
                    'agentview_image_left': torch.stack([
                        self.past_obs['agentview_image_left'], 
                        self.current_obs['agentview_image_left']
                    ]),
                    'agentview_image_right': torch.stack([
                        self.past_obs['agentview_image_right'], 
                        self.current_obs['agentview_image_right']
                    ]),
                    'eef_pos': torch.stack([
                ...
                model_input = {
                    'agentview_image_left': torch.stack([
                        current_obs['agentview_image_left'], 
                        current_obs['agentview_image_left']
                    ]),
                    'agentview_image_right': torch.stack([
                        current_obs['agentview_image_right'], 
                        current_obs['agentview_image_right']
                    ]),
                    'eef_pos': torch.stack([
                        current_obs['eef_pos'], 
            ...
            # Use joint position observations
            if self.past_obs is not None:
                model_input = {
                    'agentview_image_left': torch.stack([
                        self.past_obs['agentview_image_left'], 
    ...
    def execute_action_in_sim(self, action, i):
        ...
        if len(action_np) == 22:
            ...
            target_quat = np.array([target_quat[0], target_quat[1], target_quat[2], target_quat[3]])
