...
class RealSystemDiffusionPolicy:
    ...
    def prepare_model_input(self):
        ...
        if ee_pos is None:
            return None
        ee_quat = self.mat_to_quat.forward(torch.tensor(ee_rot[None], dtype=torch.float32, device=self.device))[0].cpu().numpy() # wxyz
        ...
        if self.mode == 'pose':
            ...
            if self.past_obs is not None:
                ...
                current_obs = {
                    ...
                    'eef_quat': torch.tensor(ee_quat, dtype=torch.float32),
    ...
    def execute_action_chunk(self, action_chunk):
        ...
        for i, action in enumerate(action_chunk):
            ...
            if len(action_np) == 22:
                ...
                target_quat = self.six_to_quat.forward(torch.tensor(target_rot_6d)[None])[0].cpu().numpy() # wxyz
