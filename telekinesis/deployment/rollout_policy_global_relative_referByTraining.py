...
class HDF5DataProcessor:
    def __init__(self, hdf5_path, demo_idx=0):
        """
        初始化HDF5数据处理器
        Args:
            hdf5_path: HDF5文件路径
    ...
    def load_hdf5_data(self):
        ...
        try:
            with h5py.File(self.hdf5_path, 'r') as f:
                demo = f['data'][f'demo_{self.demo_idx}']
                # 获取这个demo的总帧数
                self.total_steps = demo['obs']['agentview_image'].shape[0]
                # 将整个demo加载到内存
                self.demo_data = {
                    'agentview_image': np.array(demo['obs']['agentview_image']),
                    'eef_pos': np.array(demo['obs']['eef_pos']),
                    'eef_quat': np.array(demo['obs']['eef_quat']),
                    'gripper_qpos': np.array(demo['obs']['gripper_qpos'])
                }
                print(f"Successfully loaded demo_{self.demo_idx}")
                print(f"Total frames in demo: {self.total_steps}")
                print(f"Frame shapes: ")
    ...
    def get_sample(self, index=None):
        """获取指定索引的样本，如果不指定索引则返回当前步骤的样本并自动递增"""
        ...
        try:
            ...
            if index is None:
                ...
                # 确保索引在有效范围内
                index = index % self.total_steps
            
            # 获取当前帧的数据
            sample = {
                'agentview_image': self.demo_data['agentview_image'][index],
                'eef_pos': self.demo_data['eef_pos'][index],
                'eef_quat': self.demo_data['eef_quat'][index],
                'gripper_qpos': self.demo_data['gripper_qpos'][index]
            }
            
            print(f"Getting frame {index}/{self.total_steps} from demo_{self.demo_idx}")
            return sample
            
        except Exception as e:
            print(f"Error getting sample at index {index}: {e}")
            return None
...
class RealSystemDiffusionPolicy:
    def __init__(self, checkpoint_path, device='cuda:0', mode='pose', demo_idx=0):
        """
        ...
        Args:
            ...
            mode: Control mode ('pose' or 'joint')
            demo_idx: Index of the demo to use from HDF5 file
        """
        ...
        # 加载HDF5数据
        self.hdf5_processor = HDF5DataProcessor('/media/Common/leapUMI-dataset/code/training_dataset/1104_internalcamera_relative_global.hdf5', demo_idx=demo_idx)
        self.current_hdf5_index = 0
    ...
    def prepare_model_input(self):
        ...
        if self.mode == 'pose':
            # 从HDF5数据集获取参考状态
            ref_sample = self.hdf5_processor.get_sample()
            if ref_sample is None:
                rospy.logerr("Failed to get reference sample from HDF5")
                return None
            ...
            # 使用HDF5中的图像替代实际相机图像
            ref_image = self.preprocess_image(ref_sample['agentview_image'])
            gripper_qpos = torch.tensor(ref_sample['gripper_qpos'], dtype=torch.float32)
            # Prepare current observation with relative poses and reference image
            current_obs = {
                'agentview_image': ref_image,
            ...
            if self.past_obs is not None:
                ...
                # First step, use HDF5 reference data for both past and current
                model_input = {}
                model_input['agentview_image'] = torch.stack([
                    ref_image,
                    ref_image
                ...
                model_input['eef_pos'] = torch.stack([
                    torch.tensor(ref_sample['eef_pos'], dtype=torch.float32),
                    torch.tensor(relative_pos, dtype=torch.float32)
                ])
                model_input['eef_quat'] = torch.stack([
                    torch.tensor(ref_sample['eef_quat'], dtype=torch.float32),
                    self.mat_to_quat.forward(torch.tensor(relative_rot[None])).squeeze(0)
                ])
                model_input['gripper_qpos'] = torch.stack([
                    torch.tensor(ref_sample['gripper_qpos'], dtype=torch.float32),
                    torch.tensor(self.current_leaphand_states, dtype=torch.float32)
                ])

            # Update past observation
            self.past_obs = current_obs
            
            # Increment HDF5 index for next time
            self.current_hdf5_index += 1
...
def main():
    ...
    parser.add_argument('--demo-idx', type=int, default=0,
                        help='Index of the demo to use from HDF5 file')
    
    args = parser.parse_args()
    
    try:
        # Initialize and run the system
        system = RealSystemDiffusionPolicy(args.checkpoint, args.device, demo_idx=args.demo_idx)
        system.run()
