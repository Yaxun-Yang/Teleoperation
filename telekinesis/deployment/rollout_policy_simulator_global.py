...
class SimulatorDiffusionPolicy:
    ...
    def calculate_base_pose(self):
        """
        Calculate base pose from the first frame of 1113_internalcamera_abs.hdf5
        This file contains absolute pose information for base calibration
        """
        base_dataset_path = '/media/Common/leapUMI-dataset/code/training_dataset/1113_internalcamera_abs.hdf5'
        
        try:
            # Open the base calibration dataset
            print(f"Loading base pose from: {base_dataset_path}")
            base_dataset = h5py.File(base_dataset_path, 'r')
            
            # Read first frame from demo_0
            first_eef_pos = base_dataset['data']['demo_0']['obs']['eef_pos'][0]  # [x, y, z]
            first_eef_quat = base_dataset['data']['demo_0']['obs']['eef_quat'][0]  # [w, x, y, z]
            
            base_dataset.close()
            
            print(f"Setting base pose from {base_dataset_path} first frame:")
            print(f"  EEF Position: {first_eef_pos}")
            print(f"  EEF Quaternion (wxyz): {first_eef_quat}")
            
            # Convert quaternion to rotation matrix using RotationTransformer
            first_eef_quat_torch = torch.tensor(first_eef_quat, dtype=torch.float32, device=self.device)[None]
            first_eef_rot = self.quat_to_mat.forward(first_eef_quat_torch)[0].cpu().numpy()
            
            # Build base pose matrix from dataset
            self.base_mat = np.eye(4)
            self.base_mat[:3, :3] = first_eef_rot
            self.base_mat[:3, 3] = first_eef_pos
            self.previous_mat = self.base_mat.copy()
            
            print("Base pose matrix from 1113_internalcamera_abs.hdf5:")
            print(self.base_mat)
            
        except Exception as e:
            print(f"Error loading base pose from {base_dataset_path}: {e}")
    ...
    def init_pybullet(self):
        """Initialize PyBullet simulation with base pose from 1113_internalcamera_abs.hdf5"""
        ...
        # Initialize robot to base pose from 1113_internalcamera_abs.hdf5
        base_dataset_path = './training_dataset/1113_internalcamera_abs.hdf5'
        try:
            print(f"Loading initial arm qpos from: {base_dataset_path}")
            base_dataset = h5py.File(base_dataset_path, 'r')
            initial_qpos = base_dataset['data']['demo_0']['obs']['arm_qpos'][0]
            base_dataset.close()
            
            print(f"Setting PyBullet initial arm qpos from base dataset: {initial_qpos}")
            for i, joint_pos in enumerate(initial_qpos):
                p.resetJointState(self.kinova_id, i + 2, joint_pos)
        except Exception as e:
            print(f"Error loading base arm qpos: {e}, using runtime dataset")
        ...
        print(f"PyBullet simulation initialized with base pose from {base_dataset_path}")
    ...
    def convert_relative_to_absolute(self, relative_pos, relative_quat):
        ...
        absolute_pose_mat = self.base_mat @ relative_pose_mat
    ...
    def convert_absolute_to_relative(self, absolute_pos, absolute_rot):
        ...
        relative_pose_mat = np.linalg.inv(self.base_mat) @ absolute_pose_mat
...
def main():
    ...
    parser.add_argument('--dataset', type=str, 
                        default='/media/yaxun/B197/teleop_data/output/1113_internalcamera_glb.hdf5',
    ...
    parser.add_argument('--demo', type=int, default=5,
